<!DOCTYPE HTML>
<!--
	Transitive by TEMPLATED
	templated.co @templatedco
	Released for free under the Creative Commons Attribution 3.0 license (templated.co/license)
-->
<html>
	<head>
		<title>Compare Patient</title>
		<meta charset="utf-8" />
		<meta name="viewport" content="width=device-width, initial-scale=1" />
		<link rel="stylesheet" href="assets/css/main.css" />
		<script defer src="https://use.fontawesome.com/releases/v5.0.6/js/all.js"></script>
	</head>
	<body>

		<!-- Header -->
			<header id="header" >
				<div class="logo"><a href="index.html">Hospital Collaboration Resource <i class="fas fa-hospital"></i></a></div>
				<a href="#menu" class="toggle"><span>Menu</span></a>
			</header>

		<!-- Nav -->
			<nav id="menu">
				<ul class="links">
					<li><a href="index.html">Home</a></li>
					<li><a href="">Logout</a></li>
				</ul>
            </nav>

        <section id="three" class="wrapper style2">
        <div class="inner">
            <div class="grid-style">

                <div>
                    <div class="box">
                        <div class="content">
                            <header class="align-center">
                                <h2>Record Name</h2>
                                <p>Doctor Name, Hospital Name</p>
                            </header>
                            <hr>
                            <h3>Patient Information <i class="fas fa-info-circle"></i></h3>
                            <dl>
                                <dt><b>Patient Name</b></dt>
                                <dd>
                                    <p>Patient Name</p>
                                </dd>
                                <dt><b>Gender</b></dt>
                                <dd>
                                    <p>Male</p>
                                </dd>
                                <dt><b>Weight and Height</b></dt>
                                <dd>
                                    <p>65 kg, 175 km</p>
                                </dd>
                            </dl>
                            <hr>
                            <h3>Record Information <i class="fas fa-info-circle"></i></h3>
                            <dl>
                                <dt><b>Record Date</b></dt>
                                <dd>
                                    <p>March 24, 2017</p>
                                </dd>
                                <dt><b>Description</b></dt>
                                <dd>
                                    <p>kvhksdfhgiukrshfivhdgkhiglhvio'hgihdfg'sidhfgdjfgshdfghdifb</p>
                                </dd>
                                <dt><b>Prescription</b></dt>
                                <dd>
                                    <p>gshjnjghifudkbvnijhsgnfjjshgfnjghdn</p>
                                </dd>
                            </dl>

                            <hr>
                            <ul class="actions" align="center">
                                <li><a href="#" class="button icon fa-check">Done Comparison</a></li>
                                <li><a href="#" class="button alt icon fa-search">see another</a></li>
                            </ul>

                        </div>
                    </div>
                </div>

                <div>
                    <div class="box">
                        <div class="content">
                            <header class="align-center">
                                <h2>Record Name</h2>
                                <p>Doctor Name, Hospital Name</p>
                            </header>
                            <hr>
                            <h3>Patient Information <i class="fas fa-info-circle"></i></h3>
                            <dl>
                                <dt><b>Patient Name</b></dt>
                                <dd>
                                    <p>Patient Name</p>
                                </dd>
                                <dt><b>Gender</b></dt>
                                <dd>
                                    <p>Male</p>
                                </dd>
                                <dt><b>Weight and Height</b></dt>
                                <dd>
                                    <p>65 kg, 175 km</p>
                                </dd>
                            </dl>
                            <hr>
                            <h3>Record Information <i class="fas fa-info-circle"></i></h3>
                            <dl>
                                <dt><b>Record Date</b></dt>
                                <dd>
                                    <p>March 24, 2017</p>
                                </dd>
                                <dt><b>Description</b></dt>
                                <dd>
                                    <p>kvhksdfhgiukrshfivhdgkhiglhvio'hgihdfg'sidhfgdjfgshdfghdifb</p>
                                </dd>
                                <dt><b>Prescription</b></dt>
                                <dd>
                                    <p>gshjnjghifudkbvnijhsgnfjjshgfnjghdn</p>
                                </dd>
                            </dl>

                            <hr>
                            <ul class="actions" align="center">
                                <li><a href="#" class="button icon">Previous</a></li>
                                <li><a href="#" class="button alt icon">Next</a></li>
                            </ul>

                        </div>
                    </div>
                </div>

            </div>
        </div>
    </section>

        <!-- Four -->
        <section id="four" class="wrapper style3">
            <div class="inner">

                <header class="align-center">
                    <h2>Declaration of Geneva</h2>
                    <p>THE HEALTH AND WELL-BEING OF MY PATIENT will be my first consideration</p>
                </header>

            </div>
        </section>

        <!-- Scripts -->
        <script src="assets/js/jquery.min.js"></script>
        <script src="assets/js/jquery.scrolly.min.js"></script>
        <script src="assets/js/jquery.scrollex.min.js"></script>
        <script src="assets/js/skel.min.js"></script>
        <script src="assets/js/util.js"></script>
        <script src="assets/js/main.js"></script>

    </body>

</html>